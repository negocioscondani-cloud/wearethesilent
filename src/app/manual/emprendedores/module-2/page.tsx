        ...
        <div className="antialiased">
            ...
            <main className="max-w-7xl mx-auto px-6 md:px-12 py-32 space-y-20">
                <section>
                    <div className="module-header">
                        ...
                        <h1 className="text-5xl md:text-7xl font-bold serif italic mb-4">{content.module2Title}</h1>
                        <p className="text-sm text-gold/70 italic">{content.module2Label}</p>
                    </div>

                    <div className="mt-12 grid lg:grid-cols-2 gap-16 items-start">
                        <div className="space-y-6">
                            <h3 className="section-subtitle">{content.module2OptionA}</h3>
                            <p className="text-gray-300 mb-6">{content.module2OptionADesc}</p>
                            <ol className="space-y-4 text-sm text-gray-400 mb-12">
                                {content.module2OptionASteps.map((step, index) => (
                            ...
                            <h3 className="section-subtitle">{content.module2OptionB}</h3>
                            <p className="text-gray-300 mb-6">{content.module2OptionBDesc}</p>
                            <ol className="space-y-4 text-sm text-gray-400">
                                {content.module2OptionBSteps.map((step, index) => (
                                    <li key={index} className="flex items-start"><span className="step-num">{index + 1}</span> <span dangerouslySetInnerHTML={{ __html: step }}></span></li>
                                ))}
                            </ol>
                        </div>
                        <div className="relative aspect-[9/16] w-full max-w-sm mx-auto">
                            <Image src="http://studioboom.online/wp-content/uploads/2026/02/freepik__-prompt-a-highend-culinary-editorial-photograph-of__53352-scaled.avif" alt="Fotografía de producto IA" fill className="object-cover rounded-lg shadow-xl"/>
                        </div>
                    </div>
                </section>
            </main>
            ...
            <footer className="border-t border-gold/20 py-8">
                <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
                    <Button asChild variant="outline" className="text-gold border-gold/50 hover:bg-gold/10 hover:text-gold">
                       <Link href="/manual/emprendedores/module-1">
